...
export default async function EditPersonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const asVisibility = (v: unknown): "EVERYONE" | "FRIENDS" | "FAMILY" | "ONLY_ME" => {
    return v === "EVERYONE" || v === "FRIENDS" || v === "FAMILY" || v === "ONLY_ME" ? v : "EVERYONE";
  };
  const [user, person] = await Promise.all([
    getCurrentUser(),
    prisma.person.findUnique({
      where: { id },
      include: { phones: true, emails: true, privacySettings: true, tags: { include: { tag: true } } },
    }),
  ]);
  if (!person) notFound();

  const canEdit = !!user && (person.userId === user.id || user.isMaster);
  if (!canEdit) redirect(`/people/${id}`);
  ...
  const initial = {
    ...
    privacy: {
      birthDate: asVisibility(person.privacySettings?.birthDateVisibility),
      location: asVisibility(person.privacySettings?.locationVisibility),
      work: asVisibility(person.privacySettings?.workVisibility),
      maritalStatus: asVisibility(person.privacySettings?.maritalStatusVisibility),
      notes: asVisibility(person.privacySettings?.notesVisibility),
    },
    phones: person.phones.map((p) => {
      const { dialCode, national } = splitStoredPhone(p.number);
      return { id: p.id, dialCode, number: national, visibility: asVisibility((p as { visibility?: unknown }).visibility) };
    }),
    emails: person.emails.map((e) => ({ id: e.id, label: e.label ?? "", email: e.email, visibility: asVisibility((e as { visibility?: unknown }).visibility) })),
