...
export async function middleware(request: NextRequest) {
  ...
  const publicPaths = ["/lock", "/register", "/forgot-password", "/reset-password", "/verify-email"];
...
export const config = {
  matcher: ["/((?!_next|api|lock|register|forgot-password|reset-password|verify-email).*)"],
};
