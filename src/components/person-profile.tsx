    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border p-4">
        ...
        <dl className="grid gap-2 text-sm">
          <div><dt className="text-muted-foreground">{t("profile.gender")}</dt><dd>{translateGender(t, person.gender)}</dd></div>
          {showBirthDate && person.birthDate && <div><dt className="text-muted-foreground">{t("profile.birthDate")}</dt><dd>{formatDate(person.birthDate)}</dd></div>}
      ...
      {showMaritalStatus && (
        <section className="rounded-lg border p-4">
          ...
          <p className="text-sm">{t("profile.maritalStatus")}: {translateMaritalStatus(t, person.maritalStatus)}</p>
        </section>
      )}
