...
export async function addRelationship(fromPersonId: string, data: { toPersonId: string; type: RelationshipType; label?: string }) {
  ...
  if (!session?.isMaster) return { error: "Only the master admin can add relationships directly." };
...
/** Internal helper for request accept flow (bypasses master-only restriction). */
export async function addRelationshipFromRequest(fromPersonId: string, data: { toPersonId: string; type: RelationshipType; label?: string }) {
  const parsed = relationshipSchema.safeParse(data);
  if (!parsed.success) {
    return { error: parsed.error.flatten().fieldErrors as Record<string, string[] | undefined> };
  }
