...
import { groupNameSchema, groupDescriptionSchema } from "@/lib/validations";
...
export async function createGroup(formData: FormData) {
  ...
  const descriptionRaw = (formData.get("description") as string)?.trim() || undefined;
  const isPrivate = formData.get("isPrivate") === "true" || formData.get("isPrivate") === "on";
  const nameParsed = groupNameSchema.safeParse(nameRaw);
  if (!nameParsed.success) return { error: nameParsed.error.issues[0]?.message ?? "Invalid name" };
  const descParsed = groupDescriptionSchema.safeParse(descriptionRaw);
  if (!descParsed.success) return { error: descParsed.error.issues[0]?.message ?? "Invalid description" };
  const name = nameParsed.data;
  const description = descParsed.data ?? null;
